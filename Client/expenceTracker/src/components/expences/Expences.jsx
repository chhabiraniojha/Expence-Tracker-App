...
function Expences() {
    ...
    async function handleSubmit(e) {
        e.preventDefault();
        const response = await axios.post('/api/expence/addExpence', { expenceAmount, expenceDescription, expenceCategory })

        if (response.status == 200) {
            setExpences((prev) => (
                [{ expenceAmount, expenceDescription, expenceCategory }, ...prev]
            ))
            setExpenceAmount('');
                    ...
                    <div  >
                        <ul className='flex flex-1 flex-wrap p-4'>
                            {expences.map((expence) => (
                                <Expence key={expence.id} expence={expence} removeExpence={removeExpence} />
                            ))}
                        </ul>
                    </div>
                </div>
